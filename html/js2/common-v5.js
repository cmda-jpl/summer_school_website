...
function processQS5() {
  ...
  if (WF=='1') {
    wfId = getQs(queries,'wfId');    
    versionId = getQs(queries,'versionId');    
    wfRunId = getQs(queries,'wfRunId');    
    serviceId = getQs(queries,'serviceId');    
    userId1 = getQs(queries,'userId');    
  }
...
// makeQueryStr2__  for opennex, set full browser url 
function makeQueryStr2(toServer) {
  toServer = typeof toServer !== 'undefined' ? toServer:false;
  var temp1 = "";
  var temp3 = "";
  ...
  if (toServer) { 
    ...
    //temp1 += "&fromPage=" + document.location.href.split('?')[0] + '?' + temp1;
    //temp1 += "&fromPage=" + document.location.href.split('?')[0] + '?' + 'userId=%d'%0;
    temp1 += "&fromPage=" + document.location.href.split('?')[0] ;
    temp1 += "&userId=" + userId ;
